...
/**
 * Servlet para exportar todas las planillas de un curso específico en un
 * archivo Excel
 *
...
public class ExportCoursePlanillasServlet extends HttpServlet {
	...
	/**
	 * Obtener un nombre para el archivo con la planilla generada
	 *
	 * @param name      el nombre de la planilla, si es null, se usa el nombre
	 *                  "Sheet"
	 * @param maxLength la longitud máxima del nombre del archivo
	 *
	 * @return una cadena correspondiente al nombre del documento generado
	 *
	...
	/**
	 * Exportar todas las planillas de un curso específico en un archivo Excel con
	 ...
	 *
	 * @throws ServletException si ocurre un error relacionado con el servlet (e.j:
	 *                          Errores con la BD o conexion)
	 * @throws IOException      si ocurre un error de Entrada/Salida (e.j: Problemas
	 *                          con el OutputStream de la respuesta)
	 */
	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (espIdStr == null || cursoStr == null || seccion == null || periodoStr == null ||
			...
			response.sendError(HttpServletResponse.SC_BAD_REQUEST,
					"Missing parameters. Required: especialidad, curso, seccion, periodo");
			return;
		}
		...
		try {
			...
			List<PlanillaInfo> planillas = planillaDao.findPlanillasByCourse(especialidadId, promocion, seccion,
					periodo);
			...
			try {
				...
				if (esp != null)
					especialidadName = esp.getNombre();
			} catch (Exception ex) {
			...
			try (XSSFWorkbook wb = new XSSFWorkbook()) {
				for (PlanillaInfo pi : planillas) {
					...
					String sheetName = safeSheetName(
							materiaNombre != null ? materiaNombre : ("Materia-" + p.getMateriaId()), 31);
					...
					// autosize a limited number of columns (avoid very expensive autosize on many
					// columns)
				...
				String filenameBase = "Planillas_" + safeEspecialidad + "_P" + promocion + "_S" + seccion + "_PER"
						+ periodo;
				...
				response.setHeader("Content-Disposition",
						"attachment; filename=\"" + filename + "\"; filename*=UTF-8''"
								+ URLEncoder.encode(filenameBase + ".xlsx", "UTF-8"));
